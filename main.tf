...
resource "aws_route" "private_nat_gateway" {
  count = local.create_vpc && var.enable_nat_gateway && var.create_private_nat_gw_route ? local.nat_gateway_count : 0
...
resource "aws_route_table_association" "public" {
  count = local.create_vpc && var.associate_public_routes && length(var.public_subnets) > 0 ? length(var.public_subnets) : 0
